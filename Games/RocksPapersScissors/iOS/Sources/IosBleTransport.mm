...
- (instancetype)init {
    ...
    if (self) {
        ...
        NSLog(@"OnlyRps BLE: driver up, local id=%s, cached role=%s", _LocalId.c_str(),
...
- (void)resetLink {
    if (!_Linked) return;
    NSLog(@"OnlyRps BLE: net keepalive timeout -> forcing link reset");
    [self onLinkLost];
}
...
- (void)centralManagerDidUpdateState:(CBCentralManager*)central {
    ...
    if ([self shouldScan]) {
        NSLog(@"OnlyRps BLE: central powered on, scanning");
...
didFailToConnectPeripheral:(CBPeripheral*)peripheral error:(NSError*)error {
    NSLog(@"OnlyRps BLE: connect failed: %@", error);
...
- (void)peripheralManager:(CBPeripheralManager*)peripheral
    ...
    if (error) { NSLog(@"OnlyRps BLE: addService error: %@", error); return; }
