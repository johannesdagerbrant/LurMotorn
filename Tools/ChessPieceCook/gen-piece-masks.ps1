...
# the rhosgfx (CC0) SVGs, run `Tools/ImageConvert/convert-image.py --preset chess-pieces`
# first, then this cook.
#
...
#
# Usage:  powershell -ExecutionPolicy Bypass -File Tools/ChessPieceCook/gen-piece-masks.ps1
$ErrorActionPreference = 'Stop'
Add-Type -AssemblyName System.Drawing

$Root      = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)  # Tools/ChessPieceCook/ -> repo root
...
function Get-MaskAndShade([string]$png) {
    if (-not (Test-Path $png)) {
        throw "Missing content PNG: $png (run Tools/ImageConvert/convert-image.py --preset chess-pieces to author it)."
    }
...
[void]$sb.AppendLine("// Generated by Tools/ChessPieceCook/gen-piece-masks.ps1 -- do not edit by hand.")
