...
//
// Shares the pipeline layout with Sprite.*: Mvp+Tint at offsets 0/64. Text reads the
// msdfgen -pxrange (in atlas texels) from Shape.w (offset 108); the sprite Outline/
// Shape.xyz fields are unused here. InColor already carries the tint.
layout(set = 0, binding = 0) uniform sampler2D Msdf;

layout(push_constant) uniform Push {
    mat4  Mvp;
    vec4  Tint;
    vec4  Outline;   // unused by text; present for shared-layout parity
    vec4  Shape;     // .w = DistanceRange
} P;
...
float ScreenPxRange() {
    vec2 UnitRange     = vec2(P.Shape.w) / vec2(textureSize(Msdf, 0));
