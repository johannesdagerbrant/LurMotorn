...
| `rps-hud-prototype.html` | **Interactive** look-dev prototype for the RPS HUD & visual identity — open in a browser. Design lock 2026-07-19: dark "sportscar" styling; locked screenspace field/grid gradients + palette; locked icon set (as evolved in playtest: token gold, ore-pile deposits, ore-cart miner with a seated gold load, faceted rock, unrolled scroll, classic-loop scissors — CC BY sources listed in its footer); portrait HUD (opponent dropdown → gold/population/clock panel → field → 4 production plates with stack count/cost/progress); finite mines with gold bars; per-type parallel queues with stack acceleration. The palette lab and icon options stay live for future tweaks. | **LIVING** — the visual spec. Actionable issues: #84 (gameplay deltas) + #85 (HUD/render). |
